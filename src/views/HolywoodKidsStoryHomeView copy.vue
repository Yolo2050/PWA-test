<script setup>
  ...
  const playVideo = (index) => {
    if (videoPlayer[index]) {
      ...
      videoPlayer[index].requestFullscreen()
    }
  }
  ...
  const lessons = ref(lessonData.lesson)
      ...
      <div class="outer-box">
        <ul>
          <li
            ...
            :key="lesson.title"
            class="bar-box font-semibold"
          >
            <h2 class="text-center">&nbsp; Week {{ index + 1 }}</h2>
            <div class="flex justify-between space-x-4 pr-4">
              <div class="flex-grow text-center">{{ lesson.title }}</div>
              <!-- <button @click="store.goToWeekView">Go</button> -->
              <button @click="() => playVideo(index)">Go</button>
              <!-- 데모용 임시코드 -->
              <video
                :ref="
                  (el) => {
